...
def heading_from_positions(x, z):
    dx = np.diff(x, prepend=x[0]); dz = np.diff(z, prepend=z[0])
...
class KittiHeadingPairs(Dataset):
    def __init__(self, root, seq="00", win_len=60):
        ...
        for i in range(len(clean)-win_len):
            self.samples.append((noisy[i:i+win_len], clean[i:i+win_len]))
...
def train(root, out="models/heading_lstm.pt"):
    ds = KittiHeadingPairs(root); dl = DataLoader(ds,batch_size=32,shuffle=True)
    model = HeadingLSTM(); opt = torch.optim.AdamW(model.parameters(),1e-3)
    loss_fn = nn.MSELoss()
    for ep in range(5):
        loss_sum=0
        for xb,yb in dl:
            pred=model(xb); pred=pred/(pred.norm(dim=-1,keepdim=True)+1e-8)
            loss=loss_fn(pred,yb); opt.zero_grad(); loss.backward(); opt.step()
