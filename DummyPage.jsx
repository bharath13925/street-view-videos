...
import { MapPin, Route, Video, Play, Download, Clock, FileVideo, Camera, Navigation, Zap, Map as MapIcon, Image } from "lucide-react";
...
export default function Dashboard() {
  ...
  // New states for route map visualization
  const [routeMapUrl, setRouteMapUrl] = useState(null);
  const [routeMapLoading, setRouteMapLoading] = useState(false);
  const [routePolyline, setRoutePolyline] = useState(null);
  
  // Add debounce state to prevent excessive API calls
  const [cacheCheckTimeout, setCacheCheckTimeout] = useState(null);
  ...
  // Debounced cache check when locations change
  useEffect(() => {
  ...
  const generateRouteMapUrl = (routePolylineData, startLoc, endLoc, startCoords, endCoords) => {
    if (!routePolylineData) return null;
    
    const apiKey = process.env.REACT_APP_GOOGLE_MAPS_API_KEY || "YOUR_GOOGLE_MAPS_API_KEY";
    const size = "800x600";
    const mapType = "roadmap";
    
    // Create path parameter with polyline
    const path = `enc:${routePolylineData}`;
    
    // Add markers for start and end - use coordinates if available, otherwise use location names
    let startMarker, endMarker;
    
    if (startCoords && startCoords.lat && startCoords.lng) {
      startMarker = `markers=color:green|label:S|${startCoords.lat},${startCoords.lng}`;
    } else {
      startMarker = `markers=color:green|label:S|${encodeURIComponent(startLoc)}`;
    }
    
    if (endCoords && endCoords.lat && endCoords.lng) {
      endMarker = `markers=color:red|label:E|${endCoords.lat},${endCoords.lng}`;
    } else {
      endMarker = `markers=color:red|label:E|${encodeURIComponent(endLoc)}`;
    }
  ...
  const checkForCachedRoute = async () => {
    try {
      ...
      console.log("Checking for cached route...");
      ...
      const response = await fetch("http://localhost:5000/api/routes/check-existing", {
        ...
        body: JSON.stringify({
          ...
          include_map: true // Request map data
        })
      });
      ...
      if (response.ok) {
        ...
        // If route polyline is available, generate map URL
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Update route map if polyline is available
      if (genData.route_polyline) {
        setRoutePolyline(genData.route_polyline);
        const mapUrl = generateRouteMapUrl(
          genData.route_polyline, 
          startLocation, 
          endLocation, 
          startCoords, 
          endCoords
    ...
    <div className="min-h-screen bg-black text-white relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 mb-2">
          ...
          <div>
            ...
            <p className="text-gray-300 mt-1 animate-fade-in" style={{ animationDelay: '0.2s' }}>
              Generate → Smooth → Regenerate → Interpolate → Video (with AI-powered caching & route visualization)
            </p>
          </div>
        </div>
      </header>
      ...
      <main className="relative z-10 p-8 max-w-7xl mx-auto">
        ...
        <p className="mb-8 text-gray-300 text-lg animate-slide-up" style={{ animationDelay: '0.1s' }}>
          Create smooth street view sequences with LSTM smoothing, optical flow interpolation, route visualization, and video generation.
        </p>
        ...
        <form className="bg-gray-800/70 backdrop-blur-xl p-8 rounded-2xl shadow-2xl mb-8 border border-gray-700 animate-slide-up hover:shadow-lime-400/10 transition-all duration-500" style={{ animationDelay: '0.2s' }}>
          ...
          {(routeMapUrl || routeMapLoading) && (
            <div className="mb-6 animate-slide-up" style={{ animationDelay: '0.8s' }}>
              <div className="bg-gray-700/50 p-6 rounded-xl border border-gray-600 backdrop-blur-sm">
                <h3 className="text-lg font-semibold text-lime-400 mb-4 flex items-center gap-3">
                ...
                {routeMapLoading ? (
                  <div className="flex items-center justify-center p-8 bg-gray-800/50 rounded-lg">
                    ...
                    <span className="ml-3 text-gray-300">Loading route map...</span>
                  </div>
                ) : routeMapUrl ? (
                  <div className="relative rounded-lg overflow-hidden shadow-xl border border-gray-600 group">
                    <img
                      src={routeMapUrl}
                      alt="Route Map"
              ...
              {cacheStatus.cached ? (
                <div>
                  ...
                  <button
                    ...
                    className="mt-3 bg-green-600/70 hover:bg-green-600 text-white px-4 py-2 rounded-md text-sm transition-all duration-300 transform hover:scale-105 backdrop-blur-sm"
                    disabled={loading || videoLoading}
                  >
                    Use Cached Route
                  </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <div className="bg-gray-800/70 backdrop-blur-xl p-6 rounded-2xl shadow-2xl border border-gray-700 animate-slide-up hover:shadow-lime-400/10 transition-all duration-500">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-gray-300">Python Route ID:</span>
                  <span className="text-xs font-mono text-white bg-gray-700/50 px-2 py-1 rounded">{route.pythonRouteId}</span>
                </div>
                <div className="flex justify-between">
            ...
            {analytics && (
              <div className="bg-gray-800/70 backdrop-blur-xl p-6 rounded-2xl shadow-2xl border border-gray-700 animate-slide-up hover:shadow-lime-400/10 transition-all duration-500">
                ...
                <div className="grid grid-cols-2 gap-6 text-sm">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      <span className="text-gray-300">Has VO Data:</span>
                      <span className={`font-semibold ${analytics.has_vo_data ? 'text-green-400' : 'text-gray-500'}`}>
                        {analytics.has_vo_data ? 'Yes' : 'No'}
                      </span>
                    </div>
                    <div className="flex justify-between">
